...
public class LVLServer extends Thread implements Runnable {
	...
	public boolean createTRecord(String firstName, String lastName, String address, String phone, String specialization,
		...
		if (lvlDB.containsKey(lastName.substring(0, 1))) {
			...
			synchronized (lvlDB.get(lastName).subList(0, 1)) {
	...
	public static void main(String[] args) throws Exception {
		...
		alRecordInitialLVL.add(new Student("lvlFirstName", "lvlLastName", "maths", "active", "11/11/2015"));
		...
		Thread threadSocket = new Thread(new LVLServer());
		threadSocket.start();

		try {

			Endpoint.publish("http://localhost:2345/ws/" + ManagerClient.LVL_SERVER_NAME, new LVLServer());
			...
			while (true) {
				...
				if (requestContent.startsWith("ST") || requestContent.startsWith("TR")) {
					...
					case "ST":
						...
						count++;
						// adding the operation to the log file
						laval.lvlLogger.mLogger.info(" New Student record transfered with values: " + objRecord + '\n');
					...
					case "TR":
						...
						count++;
						// adding the operation to the log file
						laval.lvlLogger.mLogger.info(" New Teacher record transfered with values: " + objRecord + '\n');
						break;
					}
					...
					laval.lvlLogger.mLogger.info(" Laval server received request to send '\n'");
